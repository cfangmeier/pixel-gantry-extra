...
//    string location;
    ...
    query_component(session_id, "cmsfpix_phase2", &n_component, (int*) id, (char*) part , (char*) status, (char*) location);
    for(int i = 0; i < n_component; i++) {
        cout << id[i] <<  ", " << (char*) part << ", " << (char*) status[i] << ", " << (char*) location[i] << endl;
    }
